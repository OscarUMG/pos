        ...
        <div class="form-goup row">
          <div class="col-4">
            ...
            <input name="venta_total" id="venta_total" required type="number" oninput="ActualizaValorVentaNeto()"
              class="form-control" step="0.01" value="{{$articulo->venta_imp + $articulo->venta_neto}}">
          </div>
          <div class="col-4">
            <label>Activo</label>
        ...
        <div class="form-group row">
          <div class="col-4">
            ...
            <input name="venta_neto" id="venta_neto" required type="number" oninput="ActualizaValorVentaTotal()"
              class="form-control" step="0.01" value="{{$articulo->venta_neto}}" hidden>
          </div>
          <div class="col-4">
            {{-- <label>I.V.A.</label> --}}
            <input name="venta_imp" id="venta_imp" required type="number" class="form-control"
            step="0.01" value="{{$articulo->venta_imp}}" hidden>
          </div>
        </div>
        <div class="form-group row">
          <div class="col-6">
            ...
            step="0.01" value="{{((($articulo->venta_imp + $articulo->venta_neto)-($articulo->costo_imp + $articulo->costo_neto))/($articulo->venta_imp + $articulo->venta_neto))*100}}" hidden>
          </div>
          <div class="col-6">
            {{-- <label>Ganancia</label> --}}
            <input name="ganancia" id="ganancia" disabled type="number" class="form-control"
            step="0.01" value="{{number_format(($articulo->venta_imp + $articulo->venta_neto)-($articulo->costo_imp + $articulo->costo_neto), 0,'', '.')}}" hidden>
          </div>
        </div>
...
<script>
  ...
  function ActualizaValorCostoTotal() {
    ...
    document.getElementById("costo_total").value =  parseFloat(valor * 1.19).toFixed(2);
    document.getElementById("costo_imp").value =  parseFloat((valor * 1.19) - valor).toFixed(2);
  ...
  function ActualizaValorCostoNeto() {
    ...
    document.getElementById("costo_neto").value =  parseFloat(valor / 1.19).toFixed(2);
    document.getElementById("costo_imp").value =  parseFloat(valor - (valor / 1.19)).toFixed(2);
    ActualizaValorGanancia();
  }
  function ActualizaValorGanancia() {
    ...
    document.getElementById("ganancia").value =  parseFloat(venta_total - costo_total).toFixed(2);
    ActualizaValorGananciaPor();
  }
  function ActualizaValorGananciaPor() {
    let venta_total = document.getElementById("venta_total").value;
    let costo_total = document.getElementById("costo_total").value;
    document.getElementById("por_ganancia").value =  parseFloat(((venta_total - costo_total) / venta_total) * 100).toFixed(2);
  }
</script>
@stop
